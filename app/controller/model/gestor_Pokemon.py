...
import unicodedata
import random
# Asumo que este archivo existe porque estaba en el código nuevo
...
class gestorPokemon:
    ...
    def mostrarPokedex(self):
        ...
        # Hace SELECT para ver si existen datos en la bd
        result = self.db.select("SELECT COUNT(*) AS total FROM PokemonPokedex")

        # En caso de no existir, se vuelca la pokedex en la bd y se vuelve a llamar al metodo 
        if result[0]["total"] == 0:
            seed_pokedex()
